...
programa  : 'programa' decl+ bloco+ 'fimprog.'
			{
...
cmdLeitura  : 'leia' AP ID {
				...
				CmdLeitura _read = new CmdLeitura(id);
...
cmdEscrita  : 'escreva' AP (
	         ID {
	         	...
	         	CmdEscrita _write = new CmdEscrita(id);
	         	stack.peek().add(_write);
	         } 
	         | 
	         TEXT {
	         	CmdEscrita _write = new CmdEscrita(_input.LT(-1).getText());
	         	stack.peek().add(_write);
         	...
         	ATTR expr PF
				{
					...
					CmdExpr _attr = new CmdExpr(id, expression);
